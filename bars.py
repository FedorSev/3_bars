...
def get_closest_bar(data, longitude, latitude):
    closet_bar = min(data, key = lambda x:
    math.sqrt((x['Cells']['geoData']['coordinates'][0] - longitude) ** 2 + (x['Cells']['geoData']['coordinates'][1] - latitude) ** 2))
    return closet_bar

def show_information_for_bar(bar):
...
if __name__ == '__main__':
    ...
    while True :
        ...
        if number == 1:
            show_information_for_bar(get_biggest_bar(data))

        elif number == 2:
            show_information_for_bar(get_smallest_bar(data))

        elif number == 3:
            user_longitude = float(input('Введите вашу долготу: '))
            user_latitude = float(input('Введите вашу широту: '))
            show_information_for_bar(get_closest_bar(data, user_longitude, user_latitude))
